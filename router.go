...
func (a *App) parseRouters() *App {
	...
	simpleParseFunc := func(prefix string, simples []*router.Simple) {
	...
	dynamicParseFunc := func(prefix string, dynamics []*router.Dynamic) {
		for _, dynamic := range dynamics {
			...
			if have {
				...
				a.parsedRouters.Dynamics[pattern] = map[string]*router.Dynamic{dynamic.Method: dynamic}
			}
		}
	}
